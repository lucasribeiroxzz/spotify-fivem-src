...
local function getUserId(source)
    ...
    if user_id then return tonumber(user_id) end
    return nil
end

local function notify(source, kind, message)
    TriggerClientEvent('Notify', source, kind, message)
end
...
local function hasAnyGroup(user_id, groups)
...
local function canUseSoundCommand(user_id)
    local perms = getPermissions()
    if not perms.enabled then return true end
    return hasAnyGroup(user_id, perms.soundCommand)
end

local function canUseDjCommand(user_id)
    local perms = getPermissions()
    if not perms.enabled then return true end
    return hasAnyGroup(user_id, perms.djCommand)
end

local function getPlayerDisplayName(source, user_id)
    local name = GetPlayerName(source) or ('Player_' .. tostring(source))
    if user_id then
        local identity = vRP.getUserIdentity(user_id)
        if identity and identity.nome and identity.sobrenome then
